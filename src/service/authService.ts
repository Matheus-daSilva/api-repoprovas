...
export async function signUpService(email: string, password: string, passwordConfirmation: string) {
    ...
    if (passwordConfirmation !== password) throw {type: "invalid_input", message: "invalid password confirmation", number: 422}
...
export async function signInService(email: string, password: string) {
    ...
    if(!bcrypt.compareSync(password, respo.password)) throw { type: "invalid_password", message: "wrong password", number: 401}
